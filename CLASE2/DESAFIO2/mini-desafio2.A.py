...
# Se pide realizar un gráfico del valor del Bitcoin de los últimos 10 años, marcar con 
# un punto el valor máximo del gráfico y calcular cuándo sucedió.

# Nota:
# Usen la función pd.read_csv. Un .csv funciona prácticamente igual a un .xlsx.
# Los valores a graficar estan la columna "Open" (usando to_dict("list") podrían 
...
# archivo = pd.read_csv(carpeta_actual + "//BTC.csv")     #Leemos el .csv
...
plt.plot(dia_del_record, precio_mas_alto, 'ko', label=record_label)   #Ploteamos un punto, 'ko' le da el tipo de punto, y le asignamos el label
plt.xticks([f'{y}-01-01' for y in range(2015, 2021)]) # Mostrar el primer día de cada año
plt.grid( axis='y' )   # Mostrar una grilla horizontal
plt.legend()           # Mostrar labels
plt.show()